...
def update_apache_maven_data(data, config, update_all_versions=False):
    scraper = ApacheMavenScraper(config)
    ...
    if update_all_versions or version_constraints is None:
        ...
        versions_to_update = filter_versions(versions, version_constraints,
...
def render_apache_maven_dockerfiles(data, config, update_all_versions=False, force_update=False):
    ...
    if update_all_versions:
        ...
        versions_to_update = filter_latest_versions(versions, config.get('version_constraints'),
                                                    normalize_version=normalize_version_to_semver)

    base_repository_info = get_base_repository_info(config)

    for version in versions_to_update:
        version_files = data['versions'][version]

        for base_repository_info in base_repository_info:
...
def main(argv):
    ...
    parser.add_argument('--data-file', nargs='?', dest='data_file', default=os.path.abspath('apache_maven.yml'))
    ...
    if perform_update:
        data = update_apache_maven_data(data, config, update_all)
        write_yaml(data_file, data)

    render_apache_maven_dockerfiles(data, config, update_all, force_update)
